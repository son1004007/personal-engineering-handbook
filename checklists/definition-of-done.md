...
- deliverable_model: [`../lifecycle/03-deliverables-and-handover.md`](../lifecycle/03-deliverables-and-handover.md)
...
변경된 진실에 해당하는 산출물만 갱신한다.

- [ ] DLV-01 요구사항/추적성에서 중요한 요구사항의 구현·검증 상태를 찾을 수 있다.
- [ ] UI/interaction 변경이면 DLV-02가 실제 UI와 일치한다.
- [ ] architecture/API/data-flow/security-boundary 변경이면 DLV-03가 일치한다.
- [ ] DB/schema/query/data-contract 변경이면 DLV-04가 일치한다.
- [ ] DLV-05 source/config/migration/test code와 문서 계약이 충돌하지 않는다.
- [ ] 설치/build/deploy가 달라졌다면 DLV-06을 갱신했다.
- [ ] 운영/검수/인수 상태가 달라졌다면 DLV-07을 갱신했다.
- [ ] `NOT RUN`, `BLOCKED`, known issue, residual risk를 완료처럼 숨기지 않았다.
...
## Handover
해당되는 경우:
- [ ] 다른 사람이 설치/배포 절차를 재현할 수 있다.
- [ ] 정상/비정상 상태와 진단 진입점을 찾을 수 있다.
